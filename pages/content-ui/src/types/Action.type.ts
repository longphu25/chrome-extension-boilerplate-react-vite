/**
 * Actions and blinks specification - v2.0
 */

/**
 * Protocol identifier for the Actions protocol
 */
export type ALGORAND_ACTIONS_PROTOCOL = 'algorand-action:';

/** @internal */
export type SupportedProtocols = ALGORAND_ACTIONS_PROTOCOL;

/**
 * The `actions.json` instruct clients on what website URLs support
 * Actions and provide the mapping rules for blink urls to reach their Actions API.
 */
 ...
 */
export type ActionGetRequest = object;
...
export type TypedActionParameter<T extends ActionParameterType = ActionParameterType> =
  T extends SelectableParameterType ? ActionParameterSelectable<T> : ActionParameter<T>;
...
type GeneralParameterType = 'text' | 'email' | 'url' | 'number' | 'date' | 'datetime-local' | 'textarea';
...
export type SignMessageData = {
  ...
  domain: string;
  /** base58 string of the address requested to sign this message */
  address: string;
  ...
  issuedAt: string;
  /**
   * Chain ID compatible with CAIPs, defaults to mainnet-beta in clients.
 ...
 */
export type ActionPostResponse = TransactionResponse | PostResponse | ExternalLinkResponse | SignMessageResponse;
...
/**
 * Fields of a Action transaction request URL.
 */
export interface ActionRequestURLFields {
  /** `link` in the Action spec */
  link: URL;
  /** `label` in the Action spec */
  label?: string;
  /** `message` in the Action spec  */
  message?: string;
}

/**
 * Fields of a blink URL to support a Action.
 */
export interface BlinkURLFields {
  /** base URL for the `blink` in the Action spec */
  blink: URL;
...
export type ActionInputWithButtonProps = {
  placeholder?: string;
  name?: string;
  disabled?: boolean;
  button: ActionButtonProps;
