export const ALGO_ACTION_PREFIX = /^(algorand-action:|algo-action:|algorand:)/;
...
export function isInterstitial(url: string | URL): IsInterstitialResult {
  try {
    ...
    if (!ALGO_ACTION_PREFIX.test(urlDecodedActionUrl)) {
      return { isInterstitial: false };
    }

    const decodedActionUrl = urlDecodedActionUrl.replace(ALGO_ACTION_PREFIX, '');
